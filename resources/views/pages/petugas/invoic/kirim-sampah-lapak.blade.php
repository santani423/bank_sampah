...
@section('main')
    ...
    <div class="container mt-4 invoice-center" id="invoice-area">
        <div class="card shadow-lg border-0 mb-4 animate__animated animate__fadeIn invoice-card">
            <div class="card-header bg-gradient bg-primary d-flex align-items-center justify-content-between">
                <h3 class="mb-0 invoice-title-white"><i class="bi bi-receipt-cutoff me-2"></i>Invoice Pengiriman Sampah ke
                    Lapak</h3>
                ...
                <div class="row mb-3 justify-content-center">
                    <div class="col-md-6 mb-2">
                        <div class="p-3 rounded bg-light mb-2 h-100 d-flex flex-column justify-content-center">
                            <strong><i class="bi bi-upc-scan me-1"></i> Kode Pengiriman:</strong>
                            <span>{{ $invoice->kode_pengiriman ?? '-' }}</span><br>
                            <strong><i class="bi bi-calendar-event me-1"></i> Tanggal:</strong>
                            <span>{{ $invoice->tanggal_pengiriman ?? '-' }}</span><br>
                            <strong><i class="bi bi-person-badge me-1"></i> Petugas:</strong>
                            <span>{{ $invoice->petugas->nama ?? '-' }}</span><br>
                        </div>
                    </div>
                    <div class="col-md-6 mb-2">
                        <div class="p-3 rounded bg-light mb-2 h-100 d-flex flex-column justify-content-center">
                            <strong><i class="bi bi-shop-window me-1"></i> Lapak Tujuan:</strong>
                            <span>{{ $invoice->gudang->nama_gudang ?? '-' }}</span><br>
                            <strong><i class="bi bi-geo-alt me-1"></i> Alamat Lapak:</strong>
                            <span>{{ $invoice->gudang->alamat ?? '-' }}</span><br>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @foreach ($invoice->detailPengirimanLapaks ?? [] as $i => $detail)
            <div class="card shadow-lg border-0 mb-4 animate__animated animate__fadeInUp invoice-card">
                <div class="card-header bg-gradient bg-success d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 invoice-kode-white"><i
                            class="bi bi-box-seam me-2"></i>{{ $detail->transaksiLapak->kode_transaksi ?? '-' }}</h4>
                    <span class="badge bg-light text-success fs-6"><i class="bi bi-arrow-right-circle me-1"></i>
                        Transaksi</span>
                </div>
                <div class="card-body">
                    <div class="row mb-3 justify-content-center">
                                ...
                                @foreach ($detail->transaksiLapak->detailTransaksiLapak ?? [] as $j => $transaksiDetail)
                                    <tr>
                                        ...
                                        <td class="text-end">Rp
                                            {{ number_format($transaksiDetail->harga_per_kg ?? 0, 0, ',', '.') }}</td>
                                        <td class="text-end fw-bold">Rp
                                            {{ number_format($transaksiDetail->total_harga ?? 0, 0, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
    ...
    <div class="container mb-5 d-flex justify-content-center">
        <a class="btn btn-danger btn-lg shadow" id="btnPrintPDF" target="_blank">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (kode && btn) {
                ...
                btn.onclick = function(e) {
                    e.preventDefault();
                    alert('Kode pengiriman tidak ditemukan!');
                };
            }
        });
    </script>
